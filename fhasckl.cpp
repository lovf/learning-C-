...
2.杨辉三角:
vector<  vector<int> >  //等于一个里面一个外面

a[i][j]--->vector<vector<int>>--->operator[](i).operator[](j)-->vector<int> 二维矩阵的访问形式
...
对于范围for而言,假如遍历到最后一个没有空间的元素那么就会出错误.
vector:泛型的编程
顺序表实现:
